(function($) {
	...
	ScrollCarousel.prototype.buttonGenerator = function() {
		var _ = this;

		var onIndicatorClick = function(_index, item) {
			var $item = $(item),
				scrollAdjust,
				itemMargin;
			...
			_.animateScroll(scrollAdjust);
		}

		if (_.options.buttonType === 'arrows') {
			$('<button/>', {
				class: "sc__next-button sc__button",
				text: "next",
				click: function() {
			...
			$('<button/>', {
				class: "sc__prev-button sc__button",
				text: "prev",
				click: function() {
			...
			$('<div/>', {
				class: "sc__indicator-container"
			}).appendTo(_.$carousel);
			_.$carouselItems.each(function(_index, item) {
				$('<button/>', {
					class: "sc__indicator-button",
					text: _index + 1,
					click: function() {
						onIndicatorClick(_index, item)
					}
				}).wrapInner('<span ' + (_.options.showIndicatorNumbers ?  '' : 'class="visuallyhidden"') + ' />').appendTo($('.sc__indicator-container'));
			});
		}
	}

    ScrollCarousel.prototype.animateScroll = function(position) {
    	var _ = this;

    	if (_.options.disableAnimation) {
    		_.$carousel.scrollLeft(position);
    	}
    	else {
    		_.$carousel.animate({
				scrollLeft: position
...
$('.sc__carousel').scrollCarousel({buttonType: 'indicators'});
